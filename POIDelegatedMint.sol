...
contract POIDelegatedMint {

    event VerifiedSignature(
        bytes _signedMessage
    );

    string public name = "Proof of Impact";

    // POI Contract - Only Mint Token function
    POI POIToken;

    // Contract creator
    address public owner;

    // POI valid token minter
    address public validSigner;

    // Processed signatures
    mapping(bytes => bool) public processed;

    constructor (address _poiContractAddress, address _validSigner) public{
        validSigner = _validSigner;
        POIToken = POI(_poiContractAddress);
        owner = msg.sender;
    }
    ...
    function mintToken(uint256 event_id, address receiver, bytes memory signedMessage) public returns (bool) {
        ...
        return POIToken.mintToken(event_id, receiver);
    }
}
